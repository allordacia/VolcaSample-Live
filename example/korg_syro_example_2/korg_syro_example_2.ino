...
/*
>>NOTE - it has only been tested with the sample
  file that came with the library which uses this
  command:
  volcasample syro_stream.wav "s2c:02 Kick 3.wav"

_2
  This is now generating audio!

  Modify it to write the output to the
  audio board headphones/lineout
  Unfortunately it appears that I have to use SD instead
  of SDFat so the input function has been rewritten

_1
  This now generates the same wav file as the PC version
*/
...
AudioInputI2S            i2s2;           //xy=334.8833312988281,245.88333129882812
AudioPlayQueue           queue_l;         //xy=443.8833312988281,382.8833312988281
AudioPlayQueue           queue_r;         //xy=444.8833312988281,342.8833312988281
AudioOutputI2S           i2s1;           //xy=654.8833312988281,337.8833312988281
AudioConnection          patchCord1(queue_l, 0, i2s1, 0);
AudioConnection          patchCord2(queue_r, 0, i2s1, 1);
AudioControlSGTL5000     sgtl5000_1;     //xy=343.8833312988281,458.0000305175781
...
// pointers to the left and right channel audio buffers
uint8_t *bufl_dest, *bufr_dest;
...
static int analyze_commandline(int num_of_argv, char **argv, SyroData *syro_data)
{
  ...
  for (i=0; i<num_of_argv; i++) {
    ...
    dat = argv[i][pos++];
    Serial.printf(" Input data %d dat = %c: \n", (i+1),dat);
...
static int main2(int argc, char **argv)
{
  ...
  while(!SD.begin(SD_CS_PIN)) {
    Serial.printf("SD initialization failed - retry in 10 seconds\n");
    delay(10000);
  }
...
// Modified to write a left and right channel buffer
  while (frame >= 128) {
    // Get a buffer for each channel
    while((bufl_dest = (uint8_t *)queue_l.getBuffer()) == NULL);
    while((bufr_dest = (uint8_t *)queue_r.getBuffer()) == NULL);
    // Need to do twice as many for audio because
    // we're writing separate channels
    for(int i = 0; i < 128; i++) {
      SyroVolcaSample_GetSample(handle, &left, &right);
      *bufl_dest++ = (uint8_t)left;
      *bufl_dest++ = (uint8_t)(left >> 8);
      *bufr_dest++ = (uint8_t)right;
      *bufr_dest++ = (uint8_t)(right >> 8);
      frame--;
    }
  ...
  if(frame) {
    ...
    // write the last pair of buffers
    queue_l.playBuffer();
    queue_r.playBuffer();
  }
...
/*----------------------------------------------------------------------------
  Main (Wrapper)
 ----------------------------------------------------------------------------*/
// This version doesn't create an output file but the
// output filename must be specified here anyway.
